...
| `gemini-tts`       | Google Cloud TTS (Gemini-TTS) | 需要 reCAPTCHA token，支持 30+ 音色、多编码格式，支持多 token 随机切换与自动重试 |
...
| extra 参数    | 类型   | 默认值                  | 说明                                       |
| ------------- | ------ | ----------------------- | ------------------------------------------ |
| voice         | string | Kore                    | 音色名（支持任意值，默认多语言音色见下）   |
...
**默认音色（voice = multilingual）**：

Acherner, Achird, Algenib, Algieba, Alnilam, Aoede, Autonoe, Callirrhoe, Charon, Despina, Enceladus, Erinome, Fenrir, Gacrux, Iapetus, **Kore**, Laomedeia, Leda, Orus, Pulcherrima, Puck, Rasalgethi, Sadachbia, Sadaltager, Schedar, Sulafat, Umbriel, Vindemiatrix, Zephyr, Zubenelgenubi

更多音色和语言选项参考 [Google Cloud TTS 官方文档](https://docs.cloud.google.com/text-to-speech/docs/gemini-tts?hl=zh-cn#voice_options)。
...
注意：使用 token 参数时会直接使用该值作为请求 token，不会使用系统配置，也不触发重试和 token 轮换。
